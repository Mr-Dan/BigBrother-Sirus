...
addon:RegisterDefaults("profile", {
  ...
  Toyrailroad = false,
  TemporalCascade = false,
  ReverseCascade = false,
  GuardianHand = false,
  OilCheck = true,
})
...
--[[    	reporttanks = {
	 ...
	 GuardianHand = {
          name  = L["Guardian Hand"],
          desc = L["Reports when a player get Guardian Hand(Val'anyr)."],
          type = 'toggle',
          get = function() return addon.db.profile.GuardianHand end,
          set = function(v) addon.db.profile.GuardianHand = v end,
          map = { [false] = "|cffff4040Disabled|r", [true] = "|cff40ff40Enabled|r" }		  
        },	
  ...
  TemporalCascade = {
          name  = L["TemporalCascade"],
          desc = L["Reports when a player get Temporal Cascade."],
          type = 'toggle',
          get = function() return addon.db.profile.TemporalCascade end,
          set = function(v) addon.db.profile.TemporalCascade = v end,
          map = { [false] = "|cffff4040Disabled|r", [true] = "|cff40ff40Enabled|r" }		  
        },	
  ReverseCascade = {
          name  = L["ReverseCascade"],
          desc = L["Reports when a player get Reverse Cascade."],
          type = 'toggle',
          get = function() return addon.db.profile.ReverseCascade end,
          set = function(v) addon.db.profile.ReverseCascade = v end,
          map = { [false] = "|cffff4040Disabled|r", [true] = "|cff40ff40Enabled|r" }		  
        },			
 	  }, }, -- end events
       eventsoutput = {
        ...
        checks = {
          ...
          args = {
			...
			Oil = {
              name  = L["Illusionary Oil"],
              desc = L["Illusionary Oil"],
              type = 'toggle',
              get = function() return addon.db.profile.OilCheck end,
              set = function(v) addon.db.profile.OilCheck = v end,
              map = { [false] = "|cffff4040Disabled|r", [true] = "|cff40ff40Enabled|r" }
            },
...
function addon:ConsumableCheck(Where,Full)
  ...
  local MissingOilList={}

  if not (self.db.profile.CheckFlasks or self.db.profile.CheckElixirs or self.db.profile.CheckFood or self.db.profile.OilCheck) then
  ...
  for unit in RL:IterateRoster(false) do
    if self.db.profile.Groups[unit.subgroup] then
	  ...
	  table.insert(MissingOilList,unit.name)
    end
  end
  ...
  if self.db.profile.CheckElixirs then
	...
	if self.db.profile.OilCheck then
		for i, v in ipairs(vars.oil) do
			local spellName, spellIcon = unpack(v)
			local t = self:BuffPlayerList(spellName, MissingOilList)
		end
		self:SendMessageList(L["No Oil Buff"], MissingOilList, Where)
	end  
...
function addon:COMBAT_LOG_EVENT_UNFILTERED(timestamp, subevent, srcGUID, srcname, srcflags, dstGUID, dstname, dstflags, spellID, spellname, spellschool, extraspellID, extraspellname, extraspellschool, auratype, ...)  
  ...
  if self.db.profile.GroupOnly and 
	...
	elseif self.db.profile.Firalar and is_playerdst and subevent == "SPELL_AURA_APPLIED" and (spellID == 308671) then
		...
		elseif self.db.profile.TemporalCascade and is_playerdst and subevent == "SPELL_AURA_APPLIED" and (spellID == 317158) then -- темпоральный каскад
		if self.db.profile.PolyOut[1] then
			self:Print(L["%s imposes %s"]:format("|cffff4040"..dstname.."|r",GetSpellLink(spellID)))
		end
		sendspam(L["%s imposes %s"]:format(dstname,GetSpellLink(spellID)),addon.db.profile.PolyOut)
		
		elseif self.db.profile.ReverseCascade and is_playerdst and subevent == "SPELL_AURA_APPLIED" and (spellID == 317160) then -- обратный каскад
		if self.db.profile.PolyOut[1] then
			self:Print(L["%s imposes %s"]:format("|cffff4040"..dstname.."|r",GetSpellLink(spellID)))
		end
		sendspam(L["%s imposes %s"]:format(dstname,GetSpellLink(spellID)),addon.db.profile.PolyOut)
...
--тринкеты
	...
	elseif self.db.profile.GuardianspiritRemoved and is_playersrc and subevent == "SPELL_AURA_REMOVED" and (spellID == 47788) then --оберегающий дух (ангелок)
		...
		sendspam(L["%s falls off %s"]:format(GetSpellLink(spellID), dstname),addon.db.profile.PolyOut)	
	elseif self.db.profile.GuardianHand and is_playersrc and subevent == "SPELL_AURA_APPLIED" and (spellID == 313104) then --Вал'анир
		if self.db.profile.PolyOut[1] then
			self:Print(L["%s cast %s on %s"]:format("|cff40ff40"..srcname.."|r", GetSpellLink(spellID), "|cffff4040"..dstname.."|r"))
		end		
		sendspam(L["%s cast %s on %s"]:format(srcname, GetSpellLink(spellID), dstname),addon.db.profile.PolyOut)		
